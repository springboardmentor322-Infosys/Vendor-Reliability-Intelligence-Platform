...
async def get_exceptions(
    ...
    # 1. Fetch Disputed Invoices
    res = await db.execute(select(Invoice).options(selectinload(Invoice.vendor)).where(Invoice.status == 'disputed').limit(50))
    invs = res.scalars().all()
    for i in invs:
        exceptions.append({
            'origin': 'Invoice Disputed',
            'entity_type': 'Invoice',
            'link_id': i.invoice_number,
            'date': str(i.created_at.date() if i.created_at else i.invoice_date)
        })
        
    # 2. Fetch Flagged POs
    res2 = await db.execute(select(PurchaseOrder).where(PurchaseOrder.status.in_(['flagged', 'cancelled'])).limit(50))
    pos = res2.scalars().all()
    for p in pos:
        exceptions.append({
            'origin': 'PO Integrity Flag',
            'entity_type': 'Purchase Order',
            'link_id': p.po_number,
            'date': str(p.created_at.date() if p.created_at else p.order_date)
        })
        
    # 3. Fetch Bad Audits
    res3 = await db.execute(select(AuditLog).where(AuditLog.action.ilike('%violation%')).limit(50))
    logs = res3.scalars().all()
    for l in logs:
        exceptions.append({
            'origin': 'Compliance Violation',
            'entity_type': 'System Audit',
            'link_id': f"Log-{l.id}",
            'date': str(l.timestamp.date() if l.timestamp else '')
        })
