...
def expiring_soon(days: int = 30, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    query = (
        ...
        .filter(
            ...
            models.Contract.end_date >= dt.datetime.utcnow()
        )
    )
...
def create_contract(
    ...
    create_audit_log(
        ...
        module="Contract",
        details=(
            f"Created contract '{contract.contract_title}' "
...
def update_compliance(
    ...
    previous_status = contract.compliance_status
    ...
    create_audit_log(
        ...
        details=(
            f"Updated compliance status for contract "
            f"'{contract.contract_title}' from "
...
def upload_contract_document(
    ...
    relative_path, original_name = save_upload(
        file,
        subfolder=f"contracts/{contract_id}"
    )
    ...
    create_audit_log(
        ...
        details=(
            f"Uploaded contract document "
            f"'{original_name}' for contract "
...
def download_contract_document(
    ...
    current_user: models.User = Depends(get_current_user),
        ...
        if not contract or not contract.document_path:
         raise HTTPException(
            status_code=404,
            detail="No document uploaded for this contract"
        )
        ...
        and (
            ...
            or contract.vendor_id != current_user.vendor_id
        )
    ):
         raise HTTPException(
            status_code=403,
            detail="You can only access documents for your own contracts."
        )

        return FileResponse(
        get_upload_path(contract.document_path),
