...
def _ensure_demo_users():
    """Create predictable local demo accounts without removing existing data."""
    from app.database import SessionLocal
    demo = [
        ("Admin User", "admin@gmail.com", "admin123", "Administrator"),
        ("Santhu", "santhu2711@gmail.com", "santhu123", "Vendor"),
        ("Procurement Manager", "procurement@gmail.com", "procure123", "Procurement Manager"),
        ("Supply Chain Manager", "supplychain@gmail.com", "supply123", "Supply Chain Manager"),
        ("Finance Officer", "finance@gmail.com", "finance123", "Finance Officer"),
        ("Auditor", "auditor@gmail.com", "audit123", "Auditor"),
    ]
    db = SessionLocal()
    try:
        for full_name, email, password, role in demo:
            user = db.query(models.User).filter(models.User.email == email).first()
            if user is None:
                db.add(models.User(full_name=full_name, email=email, password=hash_password(password), role=role))
            elif email in {"admin@gmail.com", "santhu2711@gmail.com"}:
...
app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=r'(https?://(localhost|127\\.0\\.0\\.1):(4200|4201)|https://.*\\.onrender\\.com)',
