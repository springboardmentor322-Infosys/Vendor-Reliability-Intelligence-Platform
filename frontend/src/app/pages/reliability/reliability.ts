...
export class ReliabilityComponent implements OnInit {
  ...
  ngOnInit(){
    this.http.get<any[]>(`${environment.apiUrl}/reliability/`).subscribe({next:r=>this.rows=r,error:e=>console.error(e)});
    this.http.get<any>(`${environment.apiUrl}/reliability/summary`).subscribe({next:r=>this.summary=r,error:e=>console.error(e)});
  }
}
